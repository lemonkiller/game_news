...
搜集游戏开发相关信息源时,遵循以下优先级:

| 倾向 | 说明 |
|------|------|
| **开源优先 (FOSS)** | 开源/自由软件工具 > 专有/商业工具。相同功能下,优先搜集开源项目的博客/ Release / 社区
| **本地优先** | 可本地运行、不依赖云服务的工具 > 纯 SaaS / 在线订阅服务。离线可用的工具和命令行工具权重更高
| **文本优先** | CLI / 配置文件 / 脚本 > GUI / 可视化编辑器。文本格式的素材和配置比二进制格式更受青睐

例如: Blender > Maya, Godot > Unity, Tiled > LDtk, LMMS > FL Studio。
上述倾向不排除收录优秀的商业工具,但在同等质量下有开源替代时优先考虑开源。
